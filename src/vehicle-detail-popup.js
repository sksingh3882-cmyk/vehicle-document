(function () {
  ...
  const VERIFICATION_NOTE = 'Note:- All information is absolutely correct and has been uploaded in the app only after verification from the mParivahan app.';
  ...
  function popupNote(showNote) {
    return showNote ? `<div style="color:#dc2626;font-size:10px;font-weight:900;line-height:1.5;margin-top:10px;margin-bottom:8px;text-align:center">${esc(VERIFICATION_NOTE)}</div>` : '';
  }

  function openPopup(vehicle, showNote = false) {
  ...
  function attach() {
    document.querySelectorAll('.upVehicle,.vehicleAlertItem').forEach(el => {
      ...
      el.addEventListener('click', e => {
        ...
        if (vehicle) { e.stopPropagation(); openPopup(vehicle, el.classList.contains('vehicleAlertItem')); }
      }, true);
    });
  }
