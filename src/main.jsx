...
const LOCAL_KEY = 'vehicle_document_expiry_app_v4';
const OLD_LOCAL_KEYS = ['vehicle_document_expiry_app_v3', 'vehicle_document_expiry_app_v2', 'vehicle_document_expiry_app'];
...
function cleanVehicle(vehicle) {
  return {
    ...
    docs: Array.isArray(vehicle.docs) && vehicle.docs.length
      ? vehicle.docs.map((item) => ({ name: item.name || 'Document', expiryDate: item.expiryDate || '' }))
...
function formatDate(dateString) {
  return dateString ? new Date(dateString).toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' }) : 'Date missing';
}
...
function App() {
  ...
  const filteredVehicles = useMemo(() => {
    ...
    return list.filter((vehicle) => [
      vehicle.vehicleNo, vehicle.model, vehicle.owner, vehicle.mobile, vehicle.lastService, vehicle.nextService, vehicle.nextServiceKm
  ...
  function addVehicle(event) {
    ...
    setForm({ vehicleNo: '', model: '', owner: '', mobile: '', lastService: '', nextService: '', nextServiceKm: '' });
    saveCloud(next, 'Cloud synced');
  }
  ...
  function addCustomDoc(vehicleId) {
    setVehicles((prev) => prev.map((vehicle) => vehicle.id === vehicleId ? touchVehicle({ ...vehicle, docs: [...(vehicle.docs || []), { name: 'New Document', expiryDate: '' }] }) : vehicle));
  }

  function removeDoc(vehicleId, index) {
    setVehicles((prev) => prev.map((vehicle) => vehicle.id === vehicleId ? touchVehicle({ ...vehicle, docs: (vehicle.docs || []).filter((_, i) => i !== index) }) : vehicle));
  }
  ...
  function sendWhatsApp(vehicle) {
    ...
    const url = (number ? 'https://wa.me/' + number : 'https://wa.me/') + '?text=' + encodeURIComponent(buildWhatsAppText(vehicle));
    window.open(url, '_blank', 'noopener,noreferrer');
  }

  function shareAllAlerts() {
    const text = totalAlerts ? allAlerts.map(({ vehicle, docItem }) => vehicle.vehicleNo + ' - ' + (vehicle.model || 'Model not added') + ' - ' + docItem.name + ': ' + formatDate(docItem.expiryDate) + ' (' + getStatus(docItem.expiryDate).label + ')').join('\n') : 'No urgent vehicle document alerts.';
  ...
  return <div className="page">
    <header className="hero">
      <div>
        ...
        <button className="syncBtn" onClick={() => saveCloud(vehicles, 'Manual cloud sync')}>Sync Now</button>
      </div>
      <button className="alertBox" onClick={shareAllAlerts}><span>Total Alerts</span><strong>{totalAlerts}</strong></button>
    </header>

    <section className="grid">
      <form className="card addCard" onSubmit={addVehicle}>
        <h2>Add Vehicle</h2>
        <input placeholder="Vehicle No. JH05AB1234" value={form.vehicleNo} onChange={(e) => updateForm('vehicleNo', e.target.value.toUpperCase())} />
        <input placeholder="Vehicle Model" value={form.model} onChange={(e) => updateForm('model', e.target.value)} />
        <input placeholder="Owner / Driver Name" value={form.owner} onChange={(e) => updateForm('owner', e.target.value)} />
        <input placeholder="WhatsApp Mobile" inputMode="tel" value={form.mobile} onChange={(e) => updateForm('mobile', e.target.value)} />
        <input type="date" title="Last Vehicle Service" value={form.lastService} onChange={(e) => updateForm('lastService', e.target.value)} />
        <input type="date" title="Next Service" value={form.nextService} onChange={(e) => updateForm('nextService', e.target.value)} />
        <input placeholder="Next Service Km" inputMode="numeric" value={form.nextServiceKm} onChange={(e) => updateForm('nextServiceKm', e.target.value)} />
      ...
      <div className="card selectedAlerts">
        ...
        {totalAlerts === 0 ? <div className="ok">No urgent alerts.</div> : allAlerts.map(({ vehicle, docItem, status }, index) => <div className="alertItem" key={vehicle.id + docItem.name + index}><div><b>{vehicle.vehicleNo} - {vehicle.model || 'Model not added'} - {docItem.name}</b><small>{formatDate(docItem.expiryDate)}</small></div><span className={'pill ' + status.tone}>{status.label}</span></div>)}
      </div>
    </section>
    ...
    <main>{filteredVehicles.length === 0 ? <div className="card empty"><b>No vehicle added.</b><span>Add your first vehicle from the form above.</span></div> : filteredVehicles.map((vehicle) => {
      ...
      return <div key={vehicle.id} className={'card vehicleCard ' + (open ? 'activeVehicle' : '')}>
        <div className="vehicleRow" onClick={() => setSelectedId(open ? null : vehicle.id)}>
          <div className="vehicleMain"><div><h2>{vehicle.vehicleNo}</h2><p>{vehicle.model ? vehicle.model + ' • ' : ''}{vehicle.owner || 'Owner not added'} {vehicle.mobile ? '• ' + vehicle.mobile : ''}</p><p>{vehicle.nextService ? 'Next Service: ' + formatDate(vehicle.nextService) : ''}{vehicle.nextServiceKm ? ' • ' + vehicle.nextServiceKm + ' km' : ''}</p></div></div>
        ...
        <div className="vehicleActions"><button className="waBtn" onClick={(e) => { e.stopPropagation(); sendWhatsApp(vehicle); }}>WhatsApp</button><button className="remove smallRemove" onClick={(e) => { e.stopPropagation(); removeVehicle(vehicle.id); }}>Delete</button></div>
        {open && <>
          <div className="editVehicle">
            <input value={vehicle.vehicleNo} onChange={(e) => updateVehicle(vehicle.id, 'vehicleNo', e.target.value)} />
            <input placeholder="Vehicle Model" value={vehicle.model || ''} onChange={(e) => updateVehicle(vehicle.id, 'model', e.target.value)} />
            <input placeholder="Owner / Driver Name" value={vehicle.owner} onChange={(e) => updateVehicle(vehicle.id, 'owner', e.target.value)} />
            <input placeholder="WhatsApp Mobile" inputMode="tel" value={vehicle.mobile} onChange={(e) => updateVehicle(vehicle.id, 'mobile', e.target.value)} />
            <input type="date" title="Last Vehicle Service" value={vehicle.lastService || ''} onChange={(e) => updateVehicle(vehicle.id, 'lastService', e.target.value)} />
            <input type="date" title="Next Service" value={vehicle.nextService || ''} onChange={(e) => updateVehicle(vehicle.id, 'nextService', e.target.value)} />
            <input placeholder="Next Service Km" inputMode="numeric" value={vehicle.nextServiceKm || ''} onChange={(e) => updateVehicle(vehicle.id, 'nextServiceKm', e.target.value)} />
          </div>
          <div className="docs">{(vehicle.docs || []).map((docItem, index) => { const status = getStatus(docItem.expiryDate); return <div className="doc" key={docItem.name + index}><div className="docTitle"><input value={docItem.name} onChange={(e) => updateDoc(vehicle.id, index, 'name', e.target.value)} /><button onClick={() => removeDoc(vehicle.id, index)}>Delete</button></div><input type="date" value={docItem.expiryDate} onChange={(e) => updateDoc(vehicle.id, index, 'expiryDate', e.target.value)} /><span className={'pill ' + status.tone}>{status.label}</span></div>; })}</div>
