...
function buildWhatsAppText(vehicle) {
  ...
  const lines = docs.map((item) => '• ' + item.name + ': ' + formatDate(item.expiryDate) + ' (' + getStatus(item.expiryDate).label + ')').join('\n');
  return 'Vehicle Document Alert\n\nVehicle: ' + vehicle.vehicleNo + '\nOwner: ' + (vehicle.owner || 'Not added') + (vehicle.mobile ? '\nMobile: ' + vehicle.mobile : '') + '\n\n' + (lines || 'No document date added.') + '\n\nPlease renew or verify documents.';
}
...
function App() {
  ...
  const filteredVehicles = useMemo(() => {
    ...
    return q ? list.filter((vehicle) => [vehicle.vehicleNo, vehicle.owner, vehicle.mobile].some((item) => (item || '').toLowerCase().includes(q))) : list;
  }, [vehicles, query]);
  ...
  function addVehicle(event) {
    ...
    const newVehicle = cleanVehicle({ id: String(Date.now()) + '-' + Math.random().toString(16).slice(2), vehicleNo: form.vehicleNo, owner: form.owner.trim(), mobile: form.mobile.trim(), docs: defaultDocs.map((item) => ({ ...item })) });
    const next = [newVehicle, ...vehicles];
    setVehicles(next); setSelectedId(newVehicle.id); setForm({ vehicleNo: '', owner: '', mobile: '' }); saveCloud(next, 'Cloud synced');
  }
  ...
  function shareAllAlerts() { const text = totalAlerts ? allAlerts.map(({ vehicle, docItem }) => vehicle.vehicleNo + ' - ' + docItem.name + ': ' + formatDate(docItem.expiryDate) + ' (' + getStatus(docItem.expiryDate).label + ')').join('\n') : 'No urgent vehicle document alerts.'; window.open('https://wa.me/?text=' + encodeURIComponent('Vehicle Document Alerts\n\n' + text), '_blank', 'noopener,noreferrer'); }
