...
function urgentDocs(vehicle){ return vehicle.docs.filter((doc)=>getStatus(doc.expiryDate).urgent); }
function validDocs(vehicle){ return vehicle.docs.filter((doc)=>{ const d=daysLeft(doc.expiryDate); return d!==null && d>30; }); }
function whatsappNumber(mobile){ const digits=(mobile||'').replace(/\D/g,''); if(!digits)return ''; return digits.length===10 ? `91${digits}` : digits; }
...
function App(){
  ...
  function addVehicle(e){
    ...
    const newVehicle={id:`${Date.now()}-${Math.random().toString(16).slice(2)}`,vehicleNo:form.vehicleNo.trim().toUpperCase(),owner:form.owner.trim(),mobile:form.mobile.trim(),docs:defaultDocs.map((doc)=>({...doc})),createdAt:new Date().toISOString()};
    setVehicles((prev)=>[newVehicle,...prev]);
    setSelectedId(newVehicle.id);
    setForm({vehicleNo:'',owner:'',mobile:''});
  }
  function removeVehicle(id){ if(!confirm('Is vehicle ko delete karna hai?'))return; setVehicles((prev)=>prev.filter((v)=>v.id!==id)); if(selectedId===id)setSelectedId(null); }
  function updateDoc(vehicleId,index,value){ setVehicles((prev)=>prev.map((vehicle)=>{ if(vehicle.id!==vehicleId)return vehicle; const docs=[...vehicle.docs]; docs[index]={...docs[index],expiryDate:value}; return {...vehicle,docs}; })); }
  function updateDocName(vehicleId,index,value){ setVehicles((prev)=>prev.map((vehicle)=>{ if(vehicle.id!==vehicleId)return vehicle; const docs=[...vehicle.docs]; docs[index]={...docs[index],name:value}; return {...vehicle,docs}; })); }
  function addCustomDoc(vehicleId){ setVehicles((prev)=>prev.map((vehicle)=>vehicle.id===vehicleId?{...vehicle,docs:[...vehicle.docs,{name:'New Document',expiryDate:''}]}:vehicle)); }
  function removeDoc(vehicleId,index){ setVehicles((prev)=>prev.map((vehicle)=>vehicle.id===vehicleId?{...vehicle,docs:vehicle.docs.filter((_,i)=>i!==index)}:vehicle)); }
  ...
  return <div className="page compactPage">
    ...
    <section className="card vehiclesTop compactTop"><h2><Car size={18}/> Vehicle List ({filteredVehicles.length})</h2><div className="search"><Search size={15}/><input placeholder="Search" value={query} onChange={(e)=>setQuery(e.target.value)}/></div></section>
    <main>{filteredVehicles.length===0?<div className="card empty"><Car size={34}/><b>Vehicle add nahi hai.</b><span>Upar form se pehla vehicle add karein.</span></div>:filteredVehicles.map((vehicle)=>{const open=selectedId===vehicle.id;const alerts=urgentDocs(vehicle).length;const valid=validDocs(vehicle).length;return <div key={vehicle.id} className={`card vehicleCard compactVehicle ${open?'activeVehicle':''}`}><div className="vehicleRow" onClick={()=>setSelectedId(open?null:vehicle.id)}><div className="vehicleMain">{open?<ChevronDown size={18}/>:<ChevronRight size={18}/>}<div><h2>{vehicle.vehicleNo}</h2><p>{vehicle.owner||'Owner not added'} {vehicle.mobile?`• ${vehicle.mobile}`:''}</p></div></div><div className="vehicleStats"><span className="miniBadge danger">{alerts} Alert</span><span className="miniBadge green">{valid} Valid</span></div></div><div className="vehicleActions"><button className="waBtn" onClick={(e)=>{e.stopPropagation();sendWhatsApp(vehicle);}}><MessageCircle size={15}/> Send</button><button className="remove smallRemove" onClick={(e)=>{e.stopPropagation();removeVehicle(vehicle.id);}}><Trash2 size={15}/> Delete</button></div>{open&&<><div className="docs compactDocs">{vehicle.docs.map((doc,index)=>{const status=getStatus(doc.expiryDate);return <div className="doc compactDoc" key={index}><div className="docTitle"><FileText size={15}/><input value={doc.name} onChange={(e)=>updateDocName(vehicle.id,index,e.target.value)}/><button onClick={()=>removeDoc(vehicle.id,index)}><Trash2 size={14}/></button></div><input type="date" value={doc.expiryDate} onChange={(e)=>updateDoc(vehicle.id,index,e.target.value)}/><span className={`pill ${status.tone}`}>{status.label}</span></div>;})}</div><button className="custom" onClick={()=>addCustomDoc(vehicle.id)}>+ Add Custom Document</button></>}</div>;})}</main>
  </div>;
}
