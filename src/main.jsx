...
function cleanVehicle(vehicle = {}) {
  return { id: vehicle.id || String(Date.now()) + '-' + Math.random().toString(16).slice(2), category: vehicle.category || 'Car', vehicleNo: (vehicle.vehicleNo || '').trim().toUpperCase(), model: vehicle.model || '', owner: vehicle.owner || '', mobile: vehicle.mobile || '', lastService: vehicle.lastService || '', nextService: vehicle.nextService || '', nextServiceKm: vehicle.nextServiceKm || '', docs: Array.isArray(vehicle.docs) && vehicle.docs.length ? vehicle.docs.map((item) => ({ name: item.name || 'Document', expiryDate: item.expiryDate || '' })) : defaultDocs.map((item) => ({ ...item })), createdAt: vehicle.createdAt || new Date().toISOString(), updatedAt: vehicle.updatedAt || new Date().toISOString() };
}
function loadLocalVehicles() { const current = safeParse(localStorage.getItem(LOCAL_KEY), null); if (Array.isArray(current)) return current.map(cleanVehicle); for (const key of OLD_LOCAL_KEYS) { const oldData = safeParse(localStorage.getItem(key), null); if (Array.isArray(oldData)) { const migrated = oldData.map(cleanVehicle); localStorage.setItem(LOCAL_KEY, JSON.stringify(migrated)); return migrated; } } return []; }
function saveLocalVehicles(vehicles) { localStorage.setItem(LOCAL_KEY, JSON.stringify(vehicles)); }
function mergeVehicles(localVehicles, cloudVehicles) { const map = new Map(); [...cloudVehicles, ...localVehicles].map(cleanVehicle).forEach((vehicle) => { const key = vehicle.id || vehicle.vehicleNo; const old = map.get(key); if (!old) return map.set(key, vehicle); const oldTime = new Date(old.updatedAt || old.createdAt || 0).getTime(); const newTime = new Date(vehicle.updatedAt || vehicle.createdAt || 0).getTime(); map.set(key, newTime >= oldTime ? vehicle : old); }); return Array.from(map.values()).sort((a, b) => (b.updatedAt || '').localeCompare(a.updatedAt || '')); }
function daysLeft(dateString) { if (!dateString) return null; const today = new Date(); today.setHours(0, 0, 0, 0); const expiry = new Date(dateString); expiry.setHours(0, 0, 0, 0); return Math.ceil((expiry - today) / 86400000); }
...
function getStatus(expiryDate) { const days = daysLeft(expiryDate); if (days === null) return { label: 'Date missing', tone: 'neutral', urgent: false, rank: 4 }; if (days < 0) return { label: 'Failed ' + Math.abs(days) + ' days ago', tone: 'danger', urgent: true, rank: 0 }; if (days === 0) return { label: 'Failed today', tone: 'danger', urgent: true, rank: 1 }; if (days <= 7) return { label: days + ' days left', tone: 'orange', urgent: true, rank: 2 }; if (days <= 30) return { label: days + ' days left', tone: 'yellow', urgent: true, rank: 3 }; return { label: days + ' days left', tone: 'green', urgent: false, rank: 5 }; }
...
function App() {
  ...
  const [form, setForm] = useState({ category: 'Car', vehicleNo: '', model: '', owner: '', mobile: '', lastService: '', nextService: '', nextServiceKm: '' });
  const remoteUpdateRef = useRef(false); const saveTimerRef = useRef(null); const firstLoadRef = useRef(true);

  async function saveCloud(nextVehicles, label = 'Cloud synced') { const clean = nextVehicles.map(cleanVehicle); saveLocalVehicles(clean); if (!supabase) { setCloudStatus('Supabase env missing - local save active'); return false; } setCloudStatus('Saving to Supabase...'); const { error } = await supabase.from('vehicles').upsert({ id: CLOUD_ROW_ID, data: { vehicles: clean }, updated_at: new Date().toISOString() }); if (error) { setCloudStatus('Supabase save failed: ' + (error.message || 'unknown')); return false; } setCloudStatus(label + ' • ' + clean.length + ' vehicle'); return true; }
  async function loadCloud() { if (!supabase) { setCloudStatus('Supabase env missing - local save active'); return; } setCloudStatus('Connecting Supabase...'); const localVehicles = loadLocalVehicles(); const { data, error } = await supabase.from('vehicles').select('data').eq('id', CLOUD_ROW_ID).maybeSingle(); if (error) { setCloudStatus('Supabase error: ' + error.message); return; } const cloudVehicles = Array.isArray(data?.data?.vehicles) ? data.data.vehicles.map(cleanVehicle) : []; const merged = mergeVehicles(localVehicles, cloudVehicles); remoteUpdateRef.current = true; setVehicles(merged); saveLocalVehicles(merged); setCloudStatus('Supabase synced • ' + merged.length + ' vehicle'); if (!data || JSON.stringify(merged) !== JSON.stringify(cloudVehicles)) await saveCloud(merged, 'Supabase merged'); }
  useEffect(() => { loadCloud().finally(() => { firstLoadRef.current = false; }); if (!supabase) return; const channel = supabase.channel('vehicle-document-sync').on('postgres_changes', { event: '*', schema: 'public', table: 'vehicles', filter: 'id=eq.main' }, (payload) => { const cloudVehicles = Array.isArray(payload.new?.data?.vehicles) ? payload.new.data.vehicles.map(cleanVehicle) : []; remoteUpdateRef.current = true; setVehicles(cloudVehicles); saveLocalVehicles(cloudVehicles); setCloudStatus('Realtime synced • ' + cloudVehicles.length + ' vehicle'); }).subscribe((status) => { if (status === 'SUBSCRIBED') setCloudStatus((prev) => prev.includes('Connecting') ? 'Supabase connected' : prev); }); return () => { supabase.removeChannel(channel); }; }, []);
  useEffect(() => { saveLocalVehicles(vehicles); if (remoteUpdateRef.current) { remoteUpdateRef.current = false; return; } if (firstLoadRef.current) return; if (saveTimerRef.current) clearTimeout(saveTimerRef.current); saveTimerRef.current = setTimeout(() => saveCloud(vehicles), 650); return () => { if (saveTimerRef.current) clearTimeout(saveTimerRef.current); }; }, [vehicles]);

  const filteredVehicles = useMemo(() => { const q = query.trim().toLowerCase(); const list = [...vehicles].sort((a, b) => urgentDocs(b).length - urgentDocs(a).length || (b.updatedAt || '').localeCompare(a.updatedAt || '')); return list.filter((vehicle) => (categoryFilter === 'All' || vehicle.category === categoryFilter) && (!q || [vehicle.category, vehicle.vehicleNo, vehicle.model, vehicle.owner, vehicle.mobile, vehicle.lastService, vehicle.nextService, vehicle.nextServiceKm].some((item) => (item || '').toLowerCase().includes(q)))); }, [vehicles, query, categoryFilter]);
  const allDocs = useMemo(() => getAllDocs(vehicles), [vehicles]);
  const alertCounts = useMemo(() => ({ red: allDocs.filter((item) => item.days !== null && item.days <= 0).length, yellow: allDocs.filter((item) => item.days !== null && item.days > 0 && item.days <= 30).length, green: allDocs.filter((item) => item.days !== null && item.days > 30).length }), [allDocs]);
  const filteredAlertDocs = useMemo(() => allDocs.filter((item) => alertFilter === 'red' ? item.days !== null && item.days <= 0 : alertFilter === 'yellow' ? item.days !== null && item.days > 0 && item.days <= 30 : item.days !== null && item.days > 30).sort((a, b) => (a.days ?? 9999) - (b.days ?? 9999)), [allDocs, alertFilter]);
  ...
  const selectedTitle = alertFilter === 'red' ? 'Red Alert - Failed Documents' : alertFilter === 'yellow' ? 'Yellow Alert - Expire in 30 Days' : 'Green Alert - Valid Documents';

  function personalPdf(vehicle) { const docRows = (vehicle.docs || []).map((doc) => '<tr><td>' + esc(doc.name) + '</td><td>' + esc(formatDate(doc.expiryDate)) + '</td><td>' + esc(getStatus(doc.expiryDate).label) + '</td></tr>').join('') || '<tr><td colspan="3">No document added</td></tr>'; const win = window.open('', '_blank'); if (!win) return alert('Popup blocked. Browser popup allow karo.'); win.document.write('<!doctype html><html><head><title>' + esc(vehicle.vehicleNo) + ' Report</title><style>body{font-family:Arial,sans-serif;margin:24px;color:#111827}h1{color:#1d4ed8;margin:0 0 8px}.top{border-bottom:3px solid #2563eb;padding-bottom:12px;margin-bottom:16px}.box{border:1px solid #dbeafe;border-radius:14px;padding:14px;margin:12px 0}p{margin:6px 0}table{width:100%;border-collapse:collapse;margin-top:10px}th,td{border:1px solid #cbd5e1;padding:8px;text-align:left;font-size:13px}th{background:#eff6ff}.footer{text-align:center;margin-top:24px;font-weight:bold;color:#475569}@media print{button{display:none}}</style></head><body><div class="top"><h1>' + esc(APP_NAME) + '</h1><p><b>Personal Vehicle PDF Report</b></p><p>Generated: ' + esc(new Date().toLocaleString('en-IN')) + '</p></div><button onclick="window.print()" style="background:#2563eb;color:white;border:0;border-radius:8px;padding:10px 14px;font-weight:bold">Save / Print PDF</button><div class="box"><p><b>Vehicle No:</b> ' + esc(vehicle.vehicleNo) + '</p><p><b>Category:</b> ' + esc(vehicle.category) + '</p><p><b>Model:</b> ' + esc(vehicle.model || 'Not added') + '</p><p><b>Owner / Driver:</b> ' + esc(vehicle.owner || 'Not added') + '</p><p><b>Mobile:</b> ' + esc(vehicle.mobile || 'Not added') + '</p><p><b>Last Vehicle Service:</b> ' + esc(formatDate(vehicle.lastService)) + '</p><p><b>Next Service:</b> ' + esc(formatDate(vehicle.nextService)) + '</p><p><b>Next Service Km:</b> ' + esc(vehicle.nextServiceKm || 'Not added') + '</p></div><table><thead><tr><th>Document</th><th>Expiry Date</th><th>Status</th></tr></thead><tbody>' + docRows + '</tbody></table><div class="footer">Thank You<br/>' + esc(APP_NAME) + '</div></body></html>'); win.document.close(); setTimeout(() => win.print(), 500); }
  function addVehicle(event) { event.preventDefault(); if (!form.vehicleNo.trim()) return alert('Enter vehicle number'); if (vehicles.some((item) => item.vehicleNo.toLowerCase() === form.vehicleNo.trim().toLowerCase()) && !confirm('This vehicle already exists. Add again?')) return; const newVehicle = cleanVehicle({ ...form, id: String(Date.now()) + '-' + Math.random().toString(16).slice(2), docs: defaultDocs.map((item) => ({ ...item })) }); const next = [newVehicle, ...vehicles]; setVehicles(next); setSelectedId(newVehicle.id); setForm({ category: 'Car', vehicleNo: '', model: '', owner: '', mobile: '', lastService: '', nextService: '', nextServiceKm: '' }); saveCloud(next, 'Cloud synced'); }
