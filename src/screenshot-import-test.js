(function () {
  ...
  function normalizeDate(raw) {
    ...
    const word = clean.match(/(\d{1,2})\s+([A-Za-z]{3,9})\s+(\d{2,4})/i) || clean.match(/([A-Za-z]{3,9})\s+(\d{1,2})\s+(\d{2,4})/i);
    if (word) {
      let day, monthName, year;
      if (/^\d/.test(word[1])) {
        day = Number(word[1]);
        monthName = word[2].toLowerCase();
        year = Number(word[3]);
      } else {
        monthName = word[1].toLowerCase();
        day = Number(word[2]);
        year = Number(word[3]);
      }
      if (year < 100) year += 2000;
  ...
  function pickDateNearKeyword(text, keywords) {
    ...
    const datePattern = /(\d{1,2}[\/\-.\s]\d{1,2}[\/\-.\s]\d{2,4}|\d{1,2}\s+[A-Za-z]{3,9}\s+\d{2,4}|[A-Za-z]{3,9}\s+\d{1,2}\s+\d{2,4})/i;
  ...
  function extractDates(text) {
    return {
      ...
      PUC: pickDateNearKeyword(text, ['puc', 'pollution', 'emission']),
      Fitness: pickDateNearKeyword(text, ['fitness', 'fit upto', 'fit up to']),
      Tax: pickDateNearKeyword(text, ['tax', 'road tax'])
    };
  }
  ...
  function addPanel() {
    ...
    const preview = panel.querySelector('.ocrPreview');
    ...
    panel.querySelector('.ocrRead').onclick = async () => {
      ...
      try {
        ...
        status.textContent = 'Preview check karo. Galat date ho to manually edit karke Confirm dabao.';
      } catch (error) {
